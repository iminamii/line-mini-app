...
function QuickTemperatureInput({
  ...
  // Highlight state for lower tier buttons
    ...
    <div id="quick-temp-panel" className="mb-4">
        ...
        >
          {Array.from({ length: 31 }, (_, i) => i * 10).map((value) => (
            <Button
              ...
              className={`flex-shrink-0 min-w-[36px] h-10 text-sm transition-all duration-200 ${
      ...
      <div id="quick-temp-lower" className="grid grid-cols-10 gap-1">
        {Array.from({ length: 10 }, (_, i) => i).map((value) => {
          const isRecorded = lastRecordedTemp !== null && selectedBaseTemp !== null && (selectedBaseTemp + value) === lastRecordedTemp
          return (
            <Button
              key={value}
              variant="outline"
              ...
              className={`h-10 text-lg font-bold transition-all duration-200 ${
                isRecorded
